...
document.querySelector('#content-grid').addEventListener('click', (event) => { // “Listen” for clicks.
    ...
    let description = block.querySelector('.block-type').textContent // grab block type FROM that block
    ...
    document.getElementById('modal-type').textContent = description || '' // same thing but for the category content block type "material, resource etc., adding more content to the modal to make it more interesting.
